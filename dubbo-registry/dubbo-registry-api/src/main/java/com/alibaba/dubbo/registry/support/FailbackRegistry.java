...
public abstract class FailbackRegistry extends AbstractRegistry {
	...
	public void register(URL url) {
		...
		try {
			// 向服务器端发送注册请求 (provider/consumer/configurators --> 注册中心)
			doRegister(url);
		} catch (Exception e) {
			Throwable t = e;

			// 如果开启了启动时检测，则直接抛出异常
			boolean check = getUrl().getParameter(Constants.CHECK_KEY, true) && url.getParameter(Constants.CHECK_KEY, true) && !Constants.CONSUMER_PROTOCOL.equals(url.getProtocol());
	...
	public void subscribe(URL url, NotifyListener listener) {
		super.subscribe(url, listener);// 存储订阅关系
		removeFailedSubscribed(url, listener);// 移除失败的订阅关系
		try {
			/**
			 * <pre>
			 * 向服务器端发送订阅请求(provider/consumer)
			...
			if (urls != null && urls.size() > 0) {
				...
				logger.error(
						"Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: "
								+ getUrl().getParameter(Constants.FILE_KEY, System.getProperty("user.home") + "/dubbo-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);
			} else {
				// 如果开启了启动时检测，则直接抛出异常
