...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
	...
	public Result invoke(final Invocation invocation) throws RpcException {
		...
		if (invokers != null && invokers.size() > 0) {
			loadbalance = ExtensionLoader.getExtensionLoader(LoadBalance.class).getExtension(
					invokers.get(0).getUrl().getMethodParameter(invocation.getMethodName(), Constants.LOADBALANCE_KEY, Constants.DEFAULT_LOADBALANCE));
		} else {
	...
	protected void checkWhetherDestroyed() {
		...
		if (destroyed) {
			throw new RpcException("Rpc cluster invoker for " + getInterface() + " on consumer " + NetUtils.getLocalHost() + " use dubbo version " + Version.getVersion()
