...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
	...
	private final Object finalizerGuardian = new Object() {
		...
		protected void finalize() throws Throwable {
			...
			if (!ReferenceConfig.this.destroyed) {
				...
				/*
				 * 先不做Destroy操作 try { ReferenceConfig.this.destroy(); } catch (Throwable t) {
	...
	private void init() {
		...
		// 2、获取类对象
		if (ProtocolUtils.isGeneric(getGeneric())) {// 泛型接口
		...
		// 3、直连提供者使用， 直连提供者只需要在消费端设置.
		String resolve = System.getProperty(interfaceName);// 1.java
															// -Dcom.alibaba.xxx.XxxService=dubbo://localhost:20890
		String resolveFile = null;
		if (resolve == null || resolve.length() == 0) {
			resolveFile = System.getProperty("dubbo.resolve.file");// 2.配置文件 java
																	// -Ddubbo.resolve.file=xxx.properties
			if (resolveFile == null || resolveFile.length() == 0) {
				File userResolveFile = new File(new File(System.getProperty("user.home")), "dubbo-resolve.properties");// 3.配置文件
		...
		// 4、相同属性的优先级consumer->module->application
		if (consumer != null) {
			if (application == null) {
	...
	private T createProxy(Map<String, String> map) {
		...
		// 2、拼装URL
		if (isJvmRefer) {// 使用本地服务,单个url
			URL url = new URL(Constants.LOCAL_PROTOCOL, NetUtils.LOCALHOST, 0, interfaceClass.getName()).addParameters(map);
			invoker = refprotocol.refer(interfaceClass, url);// 单个invoker
			...
			if (url != null && url.length() > 0) { // 用户指定URL，指定的URL可能是对点对直连地址，也可能是注册中心URL
				...
				if (us != null && us.length > 0) {
					for (String u : us) {
						...
						if (Constants.REGISTRY_PROTOCOL.equals(url.getProtocol())) {// 注册中心url
			...
			if (urls.size() == 1) {
				...
				for (URL url : urls) {
					invokers.add(refprotocol.refer(interfaceClass, url));// 多个invoker集合
				...
				if (registryURL != null) { // 有 注册中心协议的URL
					...
					invoker = cluster.join(new StaticDirectory(u, invokers));// 集群invoker
				} else { // 不是 注册中心的URL
					invoker = cluster.join(new StaticDirectory(invokers));// 集群invoker
				}
			}
		}
