...
public class DubboProtocol extends AbstractProtocol {
	...
	private ExchangeClient getSharedClient(URL url) {
		...
		ExchangeClient exchagneclient = initClient(url);// 创建新连接
	...
	private ExchangeClient initClient(URL url) {
		...
		try {
			// 设置连接应该是lazy的
			if (url.getParameter(Constants.LAZY_CONNECT_KEY, false)) {// 如果不设置，默认lazy建立连接
				client = new LazyConnectExchangeClient(url, requestHandler);// 延迟建立连接 (使用
																			// LazyConnectExchangeClient）
			} else {
				client = Exchangers.connect(url, requestHandler);// 立即建立连接 （使用 HeaderExchangeClient）
			}
		} catch (RemotingException e) {
